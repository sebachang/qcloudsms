...
class LogEntryManager(models.Manager):
    ...
    def log_create(self, instance, **kwargs):
        ...
        if changes is not None:
            ...
            biz_field = auditlog.get_model_fields(instance._meta.model)['biz_field']
            biz_id = instance
            for attr in biz_field.split('__'):
                if biz_id is not None:
                    biz_id = getattr(biz_id, attr, None)
            if biz_id is not None:
                kwargs.setdefault('biz_id', biz_id)
...
class LogEntry(models.Model):
    ...
    biz_id = models.IntegerField(default=0, null=True, blank=True)
